...
import { DsrErrorScreen } from './DsrDepositView'
...
export function DsrWithdrawView(modal: ModalProps) {
  ...
  const { stage, messages, withdraw, proceed, daiDeposit, amount, change, back } = dsrWithdraw
      ...
      <Grid gap={4}>
        ...
        {(stage === 'withdrawWaiting4Confirmation' ||
          ...
          <Box>
            ...
            <Heading
              variant="smallHeading"
              sx={{ fontWeight: 'normal', textAlign: 'center', mt: 3, mb: 5 }}
            >
              from Dai Savings Rate
            </Heading>
          </Box>
        )}
        {stage === 'withdrawFiasco' && <DsrErrorScreen message={'dsr-withdraw-error-withdrawal'} />}
      </Grid>
      {stage === 'withdrawWaiting4Confirmation' ? (
