  ...
  prepareArgs: (data, context) => {
    ...
    return [
      ...
      contract(dssProxyActionsDsr)
        .methods.join(mcdJoinDai.address, mcdPot.address, amountToWei(amount, 'DAI').toFixed(0))
        .encodeABI(),
    ]
  },
}
...
export const exit: TransactionDef<DsrExitData> = {
  ...
  prepareArgs: (data, context) => {
    ...
    return [
      dssProxyActionsDsr.address,
      contract(dssProxyActionsDsr)
        .methods.exit(mcdJoinDai.address, mcdPot.address, amountToWei(amount, 'DAI').toFixed(0))
        .encodeABI(),
    ]
  },
}
