...
export function LedgerAccountSelection({
  ...
  const [accounts, setAccounts] = useState<HWAccountInfo[]>()
  const [loadingAccounts, setLoadingAccounts] = useState<boolean>(false)
  const [error, setError] = useState()
  const mountedRef = useRef(true)

  function connectLedger(path: string) {
    if (web3Context.status !== 'connecting' && web3Context.status !== 'connectingHWSelectAccount') {
      setDerivPath(path)
      ...
      <Grid gap={3} mx={3}>
        ...
        >
          <Flex p={2} sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {liveLoading ? <Spinner size={18} /> : <Icon name="chevron_right" size={12} />}
          </Flex>
        </Button>
        <Button
          disabled={liveLoading || legacyLoading}
        ...
        >
          <Flex p={2} sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {legacyLoading ? <Spinner size={18} /> : <Icon name="chevron_right" size={12} />}
          </Flex>
        </Button>
      </Grid>
      <Button onClick={cancel} variant="textual">
